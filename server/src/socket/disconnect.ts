...
import { getIo, HOST_GRACE_MS, PLAYER_GRACE_MS, hostDisconnectTimers, playerDisconnectTimers } from './context';
...
export function registerDisconnectHandler(socket: Socket) {
  ...
  socket.on('disconnect', () => {
    ...
    if (game.hostSocketId === socket.id) {
      ...
      getIo().to(game.pin).emit('host_reconnecting');
      const timer = setTimeout(() => {
        hostDisconnectTimers.delete(game.pin);
        gm.removeSocket(socket.id);
        getIo().to(game.pin).emit('host_disconnected');
      ...
      if (!player) return;
      const sid = socket.id;
      getIo().to(`host:${game.pin}`).emit('player_reconnecting', { name: player.name });
      const timer = setTimeout(() => {
        playerDisconnectTimers.delete(sid);
        const removed = gm.removeSocket(sid);
        if (!removed) return; // already handled by rejoin
        getIo().to(`host:${removed.game.pin}`).emit('player_left', {
